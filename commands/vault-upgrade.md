...
2. **교훈 대장**: `00-meta/lessons.md`가 없으면 템플릿 `lessons.md`를 `{{DATE}}` 치환해 생성하라 — 자기개선 루프가 이 파일 존재로 켜진다. 이미 있는데 `## 기각 대장` 섹션이 없으면(v0.8.2 이하 생성분) 템플릿의 해당 섹션을 파일 끝에 멱등 추가하라 — 기존 교훈 줄은 건드리지 마라.
