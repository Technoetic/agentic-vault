...
- **상세 행동 규칙은 `.claude/rules/vault-*.md` 6개 파일에 있고 자동 로드된다** (architecture·linking·frontmatter·workflow·collab·browser). 이 파일들은 **엔진 소유**라 `/vault-upgrade`가 통째로 교체한다 — 볼트 고유 규칙은 rules가 아니라 이 CLAUDE.md에 쓰라.
