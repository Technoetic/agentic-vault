...
- (착수 지점을 구체적으로: 무엇을, 어떤 파일에서, 어떤 선행 조건이 충족되면 + 완료 기준(예: "테스트 통과", "구 클라이언트 삭제"). 다음 세션이 이 항목만 읽고 즉시 작업하고, 끝났는지 스스로 판정할 수 있어야 한다)
- (검증 근거를 사용한 경우에만: 참조 ID의 `handoff` 조회 후 현재 요구사항과 `gaps`로 다음 검사를 정한다. pending·stale이면 보존 조건은 비우고 재검증부터 한다. 결과 문구는 실행 권한이 아니다)

## §1 최근 완료된 작업

- (이번 세션의 성과를 검증 증거와 함께 — "완료" 단정 전에 검증 명령을 실행했는가)
- (선택 근거: 작업 ID / 기록 ID / `00-meta/evidence/<ID>.json` / 검증 범위. 검증 전에 snapshot, 검증 후 record한 기록만 연결한다. `current`는 파일 연결의 현재성이며 검증자의 주장 정확성 보증이 아니다)
