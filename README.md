...
| 🟡 **관리성** (exit 0) | 데드링크 · 고아/준고아 노드 · 인덱스 미등록 · 프런트매터 과대 · rules 무결성 · **세션 주입 토큰 예산 초과** · 읽기 실패 노트(§14, v0.8.4) · 노화 문서(선택) · SSOT 사실 모순(선택) | 리포트 누적 → **사용자 확인 후** 스텁 생성·링크 교정·아카이브 |
...
full 모드는 개별 노트의 읽기 실패(클라우드 하이드레이션·백신 잠금·저장 레이스)에 **fail-soft**다 — 못 읽은 노트만 건너뛰어 **§14 읽기 실패**(관리성)로 보고하고 검사를 계속하되, 검사 대상 전부가 읽기 실패면 검사 미수행이므로 exit 1이다(v0.8.4).
