...
`v0.16.0`은 같은 Markdown 볼트, Python 엔진, 명령 문서를 두 클라이언트에서
사용한다. 이번 릴리스의 로컬 검증은 [v0.16.0 검증 기록](verification/v0.16.0.md)에 남긴다.
...
스키마를 검증하며 실행 시 자동 집행은 제공하지 않는다. v0.15.1은 Telegram Jarvis의 Claude
실행을 보강한 보안·내구성 패치이며 Codex 진입점과 명령 대응은 바뀌지 않았다.
v0.16.0은 workflow·collab 규칙과 세션·lint·trace·recall 명령 문서에 긴 작업 규율을
더했다. Codex 진입점과 명령 대응은 그대로이며, 기존 볼트는 `$agentic-vault:agentic-vault upgrade`로 두 규칙과
생성 AGENTS.md를 갱신한다.
...
이전 Git 태그나 로컬 폴더를 사용하는 `agentic-vault-local` 설치를 공개 v0.16.0 태그로
...
갱신 후 `codex plugin list`로 v0.16.0과 활성화 상태를 확인하고 새 대화를 연다.
