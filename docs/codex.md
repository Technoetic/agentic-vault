...
`v0.15.1`은 같은 Markdown 볼트, Python 엔진, 명령 문서를 두 클라이언트에서
사용한다. 이번 릴리스의 로컬 검증은 [v0.15.1 검증 기록](verification/v0.15.1.md)에 남긴다.
...
이전 Git 태그나 로컬 폴더를 사용하는 `agentic-vault-local` 설치를 공개 v0.15.1 태그로
...
갱신 후 `codex plugin list`로 v0.15.1과 활성화 상태를 확인하고 새 대화를 연다.
