...
- [ ] **Step 1: 두 파일 작성**
- [ ] **Step 2: 기존 볼트 감지 명령들과 가드 문구 일치 확인**
- [ ] **Step 3: Commit** `feat: /vault-jarvis-setup + config jarvis 블록`
...
- [ ] **Step 1: README·버전 수정**
- [ ] **Step 2: Commit + tag `v0.3.0` + push** `feat: Vault Jarvis 계층 (v0.3.0)`
...
- [ ] **Step 1: NS vault-config 상태 확인·jarvis 블록 기입(화이트리스트는 사용자 ID 확인 후)**
- [ ] **Step 2: 사용자 — BotFather로 봇 생성, 토큰을 env로 등록** *(사용자만 가능)*
- [ ] **Step 3: 브리지 기동(백그라운드) → 사용자 첫 메시지로 숫자 ID 확보 → 화이트리스트 기입 → 재기동**
- [ ] **Step 4: 라이브 검증 — `/status` → `/brief` → `기억해:` 캡처(인박스 파일 확인) → 자유 질문(Q&A) 순서로 4종 확인, 결과를 NS log에 기록**
