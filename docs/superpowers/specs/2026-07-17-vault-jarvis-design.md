...
- 브리지가 `10-inbox/jarvis/YYYY-MM-DD HHMMSS-<정제된-update_id>.md`에 **결정론적으로** 저장(본문 + 수신 시각·채널 메타 1줄). LLM이 쓰기 경로에 개입하지 않는다.
...
**추기(2026-07-17 저녁)**: 2단계를 v0.4.0으로 당일 조기 구현 — 사용자 결정("플러그인 깔면 바로 쓸 수 있어야 한다"). 별도 스펙 대신 최소 설계 채택: `lessons.md` 대장(템플릿) + `/vault-session-end` §5 교훈 루프(의미 대조·횟수 누적·3회 승격 **제안 전용** — 자동 승격 금지가 오염 방지의 전부다). "반복 데이터가 쌓여야 설계 가능"이라던 유보 논리는 제안-승인 게이트가 데이터 없이도 오염을 막는다는 점에서 철회됨.
